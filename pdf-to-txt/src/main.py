...
def save_pdf_to_jpg(pdf : str, path : str) -> (str, List[str]):
	""" Saves the given PDF to one JPG per page to the path. 
		...
		:returns str, List[str]: Containing all file names and the base name
	"""
	filenames = []
	...
	for page_num, page in enumerate(pages):
		filename = os.path.join(path, "%s_%s.jpg" % (os.path.splitext(os.path.basename(pdf))[0], page_num))
...
def convert_image_to_text(base_name : str, images : List[str], output_path : str, add_page_separater : bool = True) -> str:
...
parser = argparse.ArgumentParser(description="Convert PDF to text via OCR on JPGs from those PDFs")
parser.add_argument("--input", type=str, required=True, help="Path pointing to a PDF or a folder with PDFs to parse")
parser.add_argument("--output", type=str, required=True, help="Path where to store the output TXTs")
parser.add_argument(
	"--silent",
	action="store_const",
	const=True,
...
if __name__ == "__main__":
	...
	with tempfile.TemporaryDirectory() as tmpdirname:
		...
		if os.path.isdir(args.input):
			logging.debug("Input is a directory, searching for PDFs")
			...
			documents.append(save_pdf_to_jpg(args.pdf, tmpdirname))
